...
func PartTwo() {
	...
	for scanner.Scan() {
		...
		if (firstLower <= secondLower && secondLower <= firstUpper) || (firstLower <= secondUpper && secondUpper <= firstUpper) {
			count += 1
        } else if (secondLower <= firstLower && firstLower <= secondUpper) || (secondLower <= firstUpper && firstUpper <= secondUpper) {
			count += 1
		}
	}
